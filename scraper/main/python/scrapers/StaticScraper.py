...
# Legacy code, not used in the current version of the application
def scrape(scraper_settings: ScraperSettings):
    timeout = settings_service.get_static_setting(scraper_settings.domain, 'timeout')

    driver = WebScraper.get_driver(scraper_settings)

    accept_cookies(scraper_settings.domain, driver, timeout)
    record_container = open_all_pages(scraper_settings.domain, driver, timeout)

    record_blocks = get_records(scraper_settings.domain, record_container, timeout)
    records = get_attributes(scraper_settings, record_blocks)
...
def accept_cookies(domain, driver, timeout):
    try:
        accept_button_method = settings_service.get_static_setting(domain, 'accept_button').items()
...
def open_all_pages(domain, driver, timeout):
    scroll_delay = settings_service.get_static_setting(domain, 'scroll_delay')
    max_pages = settings_service.get_static_setting(domain, 'max_pages')

    view_more_path = settings_service.get_static_setting(domain, 'view_more_button').items()

    record_container_path = settings_service.get_static_setting(domain, 'record_container').items()
    ...
    distraction_button_method = settings_service.get_static_setting(domain, 'distraction_button').items()
...
def get_records(domain, record_container, timeout):
    record_path = settings_service.get_static_setting(domain, 'record_path').items()
    return try_get_elements(record_container, record_path, timeout)


def get_attributes(scraper_settings, record_blocks):
    attributes = settings_service.get_static_setting(scraper_settings.domain, 'attributes').items()
    record_attributes = []
    ...
    for record_block in record_blocks:
        attribute_values = {
            ...
            'price': '',
            'link': '',
            'competitor_name': scraper_settings.domain,
            'location': scraper_settings.locale,
            'source': scraper_settings.url
        }
        ...
        if attribute_values['title'] is None:
            ...
            make_examples = settings_service.get_static_setting(scraper_settings.domain, 'makes')
            model_regex = settings_service.get_static_setting(scraper_settings.domain, 'model_regex')
...
def get_attribute_value(record_block, attribute):
    ...
    attribute_type = attribute[1].get('type')
    ...
    try:
        if attribute_type == 'text':
            ...
            alias_regex = attribute[1].get('regex')
        ...
        return attribute[1].get('default')
